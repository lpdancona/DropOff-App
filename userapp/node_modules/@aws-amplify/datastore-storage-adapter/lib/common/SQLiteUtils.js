...
var USER = datastore_1.utils.USER, isNonModelConstructor = datastore_1.utils.isNonModelConstructor, isModelConstructor = datastore_1.utils.isModelConstructor;
...
exports.implicitAuthFieldsForModel = function (model) {
    if (!model.attributes || !model.attributes.length) {
        return [];
    }
    var authRules = model.attributes.find(datastore_1.isModelAttributeAuth);
    if (!authRules) {
        return [];
    }
...
function modelCreateTableStatement(model, userModel) {
    ...
    var fields = Object.values(model.fields).reduce(function (acc, field) {
        ...
        if (datastore_1.isModelFieldType(field.type)) {
            ...
            return tslib_1.__spread(acc, ["" + columnParam_2]);
        }
        // default to TEXT
...
function modelInsertStatement(model, tableName) {
    ...
    var _a = tslib_1.__read(valuesFromModel(model), 2), paramaterized = _a[0], values = _a[1];
...
function modelUpdateStatement(model, tableName) {
    var _a = tslib_1.__read(updateSet(model), 2), paramaterized = _a[0], values = _a[1];
    var updateStatement = "UPDATE \"" + tableName + "\" SET " + paramaterized + " WHERE id=?";
    return [updateStatement, tslib_1.__spread(values, [model.id])];
}
...
exports.whereConditionFromPredicateObject = function (_a) {
...
function whereClauseFromPredicate(predicate) {
    ...
    function recurse(predicate, result, params) {
        ...
        if (datastore_1.isPredicateGroup(predicate)) {
            ...
            var groupResult = [];
            try {
                for (var groupPredicates_1 = tslib_1.__values(groupPredicates), groupPredicates_1_1 = groupPredicates_1.next(); !groupPredicates_1_1.done; groupPredicates_1_1 = groupPredicates_1.next()) {
            ...
            var _b = tslib_1.__read(exports.whereConditionFromPredicateObject(predicate), 2), condition = _b[0], conditionParams = _b[1];
            result.push(condition);
            params.push.apply(params, tslib_1.__spread(conditionParams));
        }
    }
}
...
function queryAllStatement(tableName, predicate, sort, limit, page) {
    ...
    if (predicate && predicate.predicates.length) {
        var _a = tslib_1.__read(whereClauseFromPredicate(predicate), 2), whereClause = _a[0], whereParams = _a[1];
        statement += " " + whereClause;
        params.push.apply(params, tslib_1.__spread(whereParams));
    }
    var orderByClause = orderByClauseFromSort(sort);
    statement += " " + orderByClause;
    if (limit) {
        var _b = tslib_1.__read(limitClauseFromPagination(limit, page), 2), limitClause = _b[0], limitParams = _b[1];
        statement += " " + limitClause;
        params.push.apply(params, tslib_1.__spread(limitParams));
    }
